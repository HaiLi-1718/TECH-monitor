...
export const VARIANT_FEEDS: Record<string, Record<string, ServerFeed[]>> = {
  ...
  tech: {
    ...
    biopharma: [
      ...
      { name: 'STAT Biotech', url: 'https://www.statnews.com/feed/' },
      { name: 'BioSpace News', url: 'https://www.biospace.com/news/feed/' },
      { name: 'Biotech Startups', url: gn('(biotech startup OR biopharma startup OR "drug discovery") funding OR raises when:30d') },
      { name: 'Biopharma VC', url: gn('(biotech OR biopharma OR pharma) ("series a" OR "series b" OR "venture capital" OR funding) when:30d') },
      { name: 'Clinical Trials Watch', url: gn('("clinical trial" OR "phase 2" OR "phase 3" OR "trial results" OR "trial readout") (biotech OR biopharma OR pharma) when:14d') },
