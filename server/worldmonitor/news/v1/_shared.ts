...
export function buildArticlePrompts(
  ...
  const isTechVariant = opts.variant === 'tech' || opts.variant === 'localtech';
...
- If intelligence context is provided, use it only if it relates to your chosen headline`;
  ...
  } else if (opts.mode === 'tech_insights') {
    systemPrompt = `${dateContext}
You rank recent tech/science/industry headlines by real-world impact (security incidents, major product or policy shifts, funding, research breakthroughs, market-moving corporate moves).
Respond with ONLY valid JSON (no markdown fences, no commentary before or after). Exact shape:
{"brief":"<2-4 sentences synthesizing only what is supported by your chosen headlines>","indices":[<1 to 8 distinct integers: 1-based line numbers from the numbered list below, highest priority first>]}

Hard rules:
- Each line below is ONE headline (fields on that line are title, source, URL — tab-separated).
- Every index MUST satisfy 1 ≤ index ≤ N where N is the number of lines below.
- Do not invent facts; "brief" must follow only from those headlines.
- If fewer than 3 lines exist, return fewer indices (minimum 1) and a shorter brief.${langInstruction}`;
