...
const VALID_VARIANTS = new Set(['full', 'tech', 'localtech', 'finance', 'happy', 'commodity']);
...
function normalizePublishedAt(raw: number): number {
  const now = Date.now();
  if (!Number.isFinite(raw)) return now;
  const millis = raw > 0 && raw < 1e11 ? raw * 1000 : raw;
  if (millis > now + FUTURE_TOLERANCE_MS) return now;
  if (millis < now - MAX_PAST_AGE_MS) return now - MAX_PAST_AGE_MS;
  return millis;
}
...
function parseRssXml(xml: string, feed: ServerFeed, variant: string): ParsedItem[] | null {
  ...
  for (const match of matches.slice(0, ITEMS_PER_FEED)) {
    ...
    const parsedDate = pubDateStr ? Date.parse(pubDateStr) : Date.now();
    const publishedAt = normalizePublishedAt(Number.isFinite(parsedDate) ? parsedDate : Date.now());
...
export async function listFeedDigest(
  ...
  const requestedVariant = VALID_VARIANTS.has(req.variant) ? req.variant : 'full';
  // localtech shares the same digest feed set as tech.
  const variant = requestedVariant === 'localtech' ? 'tech' : requestedVariant;
