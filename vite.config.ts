...
const RSS_PROXY_ALLOWED_DOMAINS = new Set([
  ...
  // CN tech/startup sources
  'www.leiphone.com', '36kr.com', 'www.infoq.cn', 'www.tmtpost.com',
]);
...
function llmHealthDevPlugin(): Plugin {
  return {
    name: 'llm-health-dev',
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (req.url !== '/api/llm-health' && !req.url?.startsWith('/api/llm-health?')) {
          return next();
        }
        ...
        try {
          ...
          const llmMod = await import('./server/_shared/llm');
          const healthMod = await import('./server/_shared/llm-health');

          if (diagnose) {
            const report = await llmMod.diagnoseLlmProviders();
            res.setHeader('Content-Type', 'application/json');
          ...
          const providers: Array<{ name: string; url: string; available: boolean }> = [];
          const checks: Array<Promise<void>> = [];
          const providerChain = ['ollama', 'groq', 'openrouter', 'generic'];
          for (const providerName of providerChain) {
            const creds = llmMod.getProviderCredentials(providerName);
            if (!creds) continue;
            checks.push((async () => {
              const available = await healthMod.isProviderAvailable(creds.apiUrl);
              providers.push({ name: providerName, url: creds.apiUrl, available });
            })());
          }
          ...
          res.setHeader('Content-Type', 'application/json');
          res.setHeader('Cache-Control', 'no-store');
          res.end(JSON.stringify({ available, providers, checkedAt: Date.now() }));
        } catch (error) {
          res.statusCode = 500;
          res.setHeader('Content-Type', 'application/json');
          res.setHeader('Cache-Control', 'no-store');
