...
function normalizeNewsTimestamp(raw: number): number {
  const now = Date.now();
  if (!Number.isFinite(raw)) return now;
  const millis = raw > 0 && raw < 1e11 ? raw * 1000 : raw;
  const maxFuture = now + 5 * 60 * 1000;
  const minPast = now - 365 * 24 * 60 * 60 * 1000;
  if (millis > maxFuture) return now;
  if (millis < minPast) return minPast;
  return millis;
}
...
function protoItemToNewsItem(p: ProtoNewsItem): NewsItem {
  ...
  return {
    ...
    title: p.title,
    link: p.link,
    pubDate: new Date(normalizeNewsTimestamp(p.publishedAt)),
    isAlert: p.isAlert,
    threat: p.threat ? {
      level,
...
function buildCrossCategoryNewsKey(item: NewsItem): string {
  const raw = (item.link || '').trim();
  if (raw) {
    try {
      const u = new URL(raw);
      u.hash = '';
      // Remove common tracking params so the same story URL dedups correctly.
      ['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'spm', 'from'].forEach((k) => {
        u.searchParams.delete(k);
      });
      const normalizedPath = u.pathname.replace(/\/+$/, '') || '/';
      const normalizedSearch = u.searchParams.toString();
      return `url:${u.hostname.toLowerCase()}${normalizedPath}${normalizedSearch ? `?${normalizedSearch}` : ''}`;
    } catch {
...
export class DataLoaderManager implements AppModule {
  ...
  renderNewsForCategory(category: string, items: NewsItem[]): void {
    ...
    // Strictly respect map time range (e.g. 48h / 7d). Older items (incl. preprints) appear only when range is "all".
  ...
  private async loadNewsCategory(category: string, feeds: typeof FEEDS.politics, digest?: ListFeedDigestResponse | null): Promise<NewsItem[]> {
    try {
      const panel = this.ctx.newsPanels[category];
      const isStartupsCategory = category === 'startups';
      const startupPrioritySources = new Set(['36Kr', '雷峰网', 'InfoQ 中文', '钛媒体']);
      ...
      const effectiveFeeds = isStartupsCategory
        ? [
          ...enabledFeeds.filter(f => startupPrioritySources.has(f.name)),
          ...enabledFeeds.filter(f => !startupPrioritySources.has(f.name)),
        ]
        : enabledFeeds;
      ...
      if (digest?.categories && category in digest.categories) {
        ...
        // Allow per-feed fallback for categories whose digest bucket is present
        // but temporarily empty (notably biopharma in localtech).
        if (items.length === 0 && this.isPerFeedFallbackEnabled()) {
          console.warn(`[News] Digest bucket "${category}" is empty, falling back to per-feed fetch`);
        } else {
      ...
      if (items.length === 0 && isStartupsCategory) {
        const coreStartupFeeds = effectiveFeeds.filter(f => startupPrioritySources.has(f.name));
        if (coreStartupFeeds.length > 0) {
          const coreItems = await fetchCategoryFeeds(coreStartupFeeds, { batchSize: 1 });
          if (coreItems.length > 0) {
            this.renderNewsForCategory(category, coreItems);
            this.ctx.statusPanel?.updateFeed(category.charAt(0).toUpperCase() + category.slice(1), {
              status: 'ok',
      ...
      if (items.length === 0 && category === 'startups') {
        ...
        const recoveryFeedPool = [
          ...
          ...(Array.isArray(feedBuckets.vcblogs) ? feedBuckets.vcblogs : []),
          ...(Array.isArray(feedBuckets.funding) ? feedBuckets.funding : []),
        ].filter((f, idx, arr) => arr.findIndex(x => x.name === f.name) === idx)
         .filter(f => !this.ctx.disabledSources.has(f.name));
        const recoveryFeeds = this.selectLimitedFeeds(recoveryFeedPool, Math.max(6, this.perFeedFallbackCategoryFeedLimit));
        if (recoveryFeeds.length > 0) {
          const recovered = await fetchCategoryFeeds(recoveryFeeds, { batchSize: this.perFeedFallbackBatchSize });
          if (recovered.length > 0) {
            this.renderNewsForCategory(category, recovered);
            this.ctx.statusPanel?.updateFeed(category.charAt(0).toUpperCase() + category.slice(1), {
              status: 'ok',
  ...
  async loadNews(): Promise<void> {
    ...
    const fulfilledByCategory = new Map<string, NewsItem[]>();
    ...
    if (fulfilledByCategory.size > 1) {
      ...
      for (const category of prioritized) {
        const current = fulfilledByCategory.get(category) ?? [];
        const deduped = current.filter((item) => {
          const key = buildCrossCategoryNewsKey(item);
          if (seen.has(key)) return false;
          seen.add(key);
          return true;
        });
        fulfilledByCategory.set(category, deduped);
        // Keep the last good render when dedup removes every item for this bucket
        // (avoids red "unavailable" while headlines are still visible from a prior pass).
    ...
    for (const items of fulfilledByCategory.values()) {
