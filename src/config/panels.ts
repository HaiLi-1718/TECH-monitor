...
const TECH_PANELS: Record<string, PanelConfig> = {
  ...
  // Phase-1 tech monitoring scope (fixed core set).
  'live-news': { name: 'Tech Briefing', enabled: true, priority: 1 },
  ai: { name: 'AI Focus', enabled: true, priority: 1 },
  events: { name: 'Event Timeline', enabled: true, priority: 1 },
  policy: { name: 'Policy & Regulation', enabled: true, priority: 1 },
  security: { name: 'Security Incidents', enabled: true, priority: 1 },
  biopharma: { name: 'Biopharma', enabled: true, priority: 1 },
  monitors: { name: 'Biopharma Custom Monitor', enabled: true, priority: 1 },
  'tech-monitoring-companies': { name: 'Company Watchlist', enabled: true, priority: 1 },
};
...
// ============================================
// LOCALTECH — 科技监测交付：含事件时间线、政策监管、安全事件、生物医药快讯与自定义监控等
// 融资、重点企业库默认关闭（可在面板菜单中手动打开）
// ============================================
...
export const PANEL_CATEGORY_MAP: Record<string, { labelKey: string; panelKeys: string[]; variants?: string[] }> = {
  ...
  techAi: {
    ...
    panelKeys: ['ai', 'tech', 'hardware', 'cloud', 'dev', 'github', 'producthunt', 'service-status', 'tech-readiness'],
    variants: ['tech', 'localtech'],
  },
  startupsVc: {
    labelKey: 'header.panelCatStartupsVc',
    panelKeys: ['startups', 'biopharma', 'vcblogs', 'regionalStartups', 'unicorns', 'accelerators', 'funding', 'ipo'],
    variants: ['tech', 'localtech'],
  },
  securityPolicy: {
