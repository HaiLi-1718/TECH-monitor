...
import type { ClusteredEvent, FocalPoint, MilitaryFlight, NewsItem } from '@/types';
...
export class InsightsPanel extends Panel {
  ...
  private buildRuleBasedBrief(titles: string[]): string | null {
    ...
    const focus = IS_TECH_LIKE_VARIANT
      ? 'Tech focus: AI, policy/regulation, security incidents, and funding signals.'
      : 'Global focus: conflict, policy, infrastructure, and market-moving events.';
    return `${focus}\nKey developments:\n${top}`;
  }
  ...
  private async updateFromClient(clusters: ClusteredEvent[], thisGeneration: number): Promise<void> {
    // Web-only: allow rule-based fallback even when AI providers are disabled.
    const hasAiProviders = isDesktopRuntime() || isAnyAiProviderEnabled();
    ...
    try {
      ...
      if (!worldBrief || now - this.lastBriefUpdate > InsightsPanel.BRIEF_COOLDOWN_MS) {
        ...
        if (hasAiProviders) {
          ...
          const geoContext = SITE_VARIANT === 'full'
            ...
            : '';
          result = await generateSummary(titles, (_step, _total, msg) => {
            // Show sub-progress for summarization
            this.setProgress(3, totalSteps, `Generating brief: ${msg}`);
          }, geoContext, undefined, summarizeOpts);
        } else {
          const fallback = this.buildRuleBasedBrief(titles);
          result = fallback ? { summary: fallback } : null;
        }
      ...
      // Keep panel data status "live" when clustered stories exist, even if AI brief
      // generation is unavailable. The "unavailable" state should indicate no data,
      // not just missing LLM summary output.
      this.setDataBadge(importantClusters.length > 0 ? 'live' : (worldBrief ? 'live' : 'unavailable'));
  ...
  private renderInsights(
    ...
    const briefHtml = worldBrief
      ? this.renderWorldBrief(worldBrief, {
        confidenceLevel: 'medium',
        verificationNote: 'Model-generated summary. Verify critical claims with primary sources.',
      })
      : '';
  ...
  private renderServerInsights(
    ...
    const briefHtml = insights.worldBrief
      ? this.renderWorldBrief(insights.worldBrief, {
        ...
        verificationNote: insights.verificationNote,
      })
      : '';
    const focalPointsHtml = this.renderFocalPoints();
    const convergenceHtml = this.renderConvergenceZones();
    ...
    return `
      <div class="insights-brief">
        <div class="insights-section-title">
          ${IS_TECH_LIKE_VARIANT ? '🚀 TECH BRIEF' : '🌍 WORLD BRIEF'}
