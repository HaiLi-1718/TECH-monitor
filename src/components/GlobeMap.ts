...
import { STARTUP_HUBS, CLOUD_REGIONS, TECH_HQS } from '@/config/tech-geo';
...
type GlobeMarker =
  ...
  | EarthquakeMarker | EconomicMarker | DatacenterMarker | StartupHubMarker | CloudRegionMarker | TechHQMarker | WaterwayMarker | MineralMarker
...
export class GlobeMap {
  ...
  private buildMarkerElement(d: GlobeMarker): HTMLElement {
    ...
    if (d._kind === 'conflict') {
      ...
      const c = d.tier === 'mega' ? '#06b6d4' : d.tier === 'major' ? '#0ea5e9' : '#38bdf8';
      el.innerHTML = GlobeMap.wrapHit(`<div style="font-size:10px;color:${c};text-shadow:0 0 3px ${c}88;">🚀</div>`);
      el.title = `${d.name} (${d.city})`;
    } else if (d._kind === 'cloudRegion') {
      const providerColor = d.provider === 'aws' ? '#f59e0b' : d.provider === 'gcp' ? '#34d399' : d.provider === 'azure' ? '#60a5fa' : '#a78bfa';
      el.innerHTML = GlobeMap.wrapHit(`<div style="font-size:10px;color:${providerColor};text-shadow:0 0 3px ${providerColor}88;">☁</div>`);
      el.title = `${d.name} (${d.provider.toUpperCase()})`;
    } else if (d._kind === 'techHQ') {
      const c = d.hqType === 'faang' ? '#60a5fa' : d.hqType === 'unicorn' ? '#a78bfa' : '#93c5fd';
      el.innerHTML = GlobeMap.wrapHit(`<div style="font-size:10px;color:${c};text-shadow:0 0 3px ${c}88;">🏢</div>`);
  ...
  private showMarkerTooltip(d: GlobeMarker, anchor: HTMLElement): void {
    ...
    if (d._kind === 'conflict') {
      ...
      html = `<span style="color:#06b6d4;font-weight:bold;">🚀 ${esc(d.name)}</span>` +
             `<br><span style="opacity:.7;">${esc(d.city)} · ${esc(d.country)}</span>` +
             `<br><span style="opacity:.5;">Tier: ${esc(d.tier)}</span>` +
             (typeof d.unicorns === 'number' ? `<br><span style="opacity:.5;">Unicorns: ${d.unicorns}</span>` : '') +
             (d.description ? `<br><span style="opacity:.5;white-space:normal;display:block;">${esc(d.description.slice(0, 100))}</span>` : '');
    } else if (d._kind === 'cloudRegion') {
      html = `<span style="color:#f59e0b;font-weight:bold;">☁ ${esc(d.name)}</span>` +
             `<br><span style="opacity:.7;">${esc(d.provider.toUpperCase())} · ${esc(d.country)}</span>` +
             `<br><span style="opacity:.5;">${esc(d.city)}</span>` +
             (typeof d.zones === 'number' ? `<br><span style="opacity:.5;">Availability zones: ${d.zones}</span>` : '');
    } else if (d._kind === 'techHQ') {
      html = `<span style="color:#60a5fa;font-weight:bold;">🏢 ${esc(d.company)}</span>` +
             `<br><span style="opacity:.7;">${esc(d.city)} · ${esc(d.country)}</span>` +
             `<br><span style="opacity:.5;">${esc(d.hqType)}</span>` +
             (typeof d.employees === 'number' ? `<br><span style="opacity:.5;">Employees: ${d.employees.toLocaleString()}</span>` : '') +
             (d.marketCap ? `<br><span style="opacity:.5;">Market Cap: ${esc(d.marketCap)}</span>` : '');
    } else if (d._kind === 'waterway') {
  ...
  private flushMarkersImmediate(): void {
    ...
    if (this.layers.cloudRegions) markers.push(...this.cloudRegionMarkers);
  ...
  private initStaticLayers(): void {
    ...
    this.techHQMarkers = TECH_HQS.map(hq => ({
