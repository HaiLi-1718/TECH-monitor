...
function getTechMonitoringDefaultMonitors(): Monitor[] {
  const basePresets = [
    'openai, anthropic, xai, databricks, palantir, scale ai',
    'nvidia, intel, samsung electronics, semiconductors, gpu, ai chip',
    'pfizer, roche, novartis, amgen, eli lilly, regeneron',
    'biotech, biopharma, clinical trial, fda, drug approval, medtech',
    'ipo, merger, acquisition, layoffs, export control, sanction',
    'cyber attack, vulnerability, data breach, ransomware, supply chain',
  ];

  const companyKeywords = TECH_MONITORING_COMPANIES
    .map((company) => company.name.toLowerCase())
    .filter(Boolean);

  const chunkedCompanyPresets: string[] = [];
  const companyChunkSize = 8;
  for (let i = 0; i < companyKeywords.length; i += companyChunkSize) {
    chunkedCompanyPresets.push(companyKeywords.slice(i, i + companyChunkSize).join(', '));
  }

  const presets = [...basePresets, ...chunkedCompanyPresets];

  return presets.map((keywords, index) => ({
    id: `tech-default-${index + 1}`,
    keywords: keywords.split(',').map(k => k.trim().toLowerCase()),
    color: MONITOR_COLORS[index % MONITOR_COLORS.length] ?? '#44ff88',
    name: `Preset ${index + 1}`,
  }));
}
...
export class App {
  ...
  private shouldRefreshIntelligence(): boolean {
    return !!this.state.countryBriefPage?.isVisible();
  }
  ...
  constructor(containerId: string) {
    ...
    let monitors = loadFromStorage<Monitor[]>(STORAGE_KEYS.monitors, []);
    if (monitors.length === 0) {
      monitors = getTechMonitoringDefaultMonitors();
    ...
    // Ensure core tech/startup sources are not accidentally persisted as disabled.
    const essentialTechSources = [
      '36Kr',
      '雷峰网',
      'InfoQ 中文',
      '钛媒体',
    ...
    let changed = false;
    for (const source of essentialTechSources) {
      if (disabledSources.delete(source)) changed = true;
    }
    if (changed) saveToStorage(STORAGE_KEYS.disabledFeeds, [...disabledSources]);
  ...
  private setupRefreshIntervals(): void {
    ...
    this.refreshScheduler.registerAll([
      ...
      { name: 'firms', fn: () => this.dataLoader.loadFirmsData(), intervalMs: 30 * 60 * 1000, condition: () => true },
