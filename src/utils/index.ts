export function formatTime(date: Date): string {
  ...
  const targetMs = date?.getTime?.() ?? Number.NaN;
  const lang = getCurrentLanguage();
  if (!Number.isFinite(targetMs)) return lang.startsWith('zh') ? '刚刚' : 'Just now';

  const FUTURE_SKEW_TOLERANCE_MS = 2 * 60 * 1000;
  const rawDiffMs = nowMs - targetMs;
  // Small clock skew should still render as "just now".
  const normalizedDiffMs = rawDiffMs < 0 && Math.abs(rawDiffMs) <= FUTURE_SKEW_TOLERANCE_MS
    ? 0
    : rawDiffMs;
  if (normalizedDiffMs < 0) {
    // For clearly future timestamps, avoid "in xx" phrasing in news cards.
    return new Date(targetMs).toLocaleString(lang);
  }
  const diff = Math.floor(normalizedDiffMs / 1000);
