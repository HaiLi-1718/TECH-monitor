...
import { mapThreatToAlertPriority } from '@/services/threat-classifier';
...
const ALERT_LOG_KEY = 'wm-alert-log-v1';
...
function readAlertLog(): AlertLogEntry[] {
  try {
...
function writeAlertLog(entries: AlertLogEntry[]): void {
  try {
    localStorage.setItem(ALERT_LOG_KEY, JSON.stringify(entries.slice(0, 500)));
  } catch {}
}

function logThreatItem(item: NewsItem): void {
  if (!item.threat) return;
  const level = item.threat.level;
  const id = makeAlertKey(item.title, item.source, item.link);
  const existing = readAlertLog();
  const alreadyLogged = existing.some((entry) => entry.id === id);
  ...
  const next: AlertLogEntry = {
    ...
    link: item.link,
    threatLevel: level,
    priority: mapThreatToAlertPriority(level),
    timestamp: item.pubDate.toISOString(),
    createdAt: new Date().toISOString(),
  };
  writeAlertLog([next, ...existing]);
}
