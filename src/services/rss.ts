  ...
  try {
    ...
    const parsed = Array.from(items)
      ...
      .map((item) => {
        ...
        const threat = classifyByKeyword(title, 'localtech');
    ...
    for (const item of aiCandidates) {
      ...
      classifyWithAI(item.title, 'localtech').then((aiResult) => {
...
export async function fetchCategoryFeeds(
  ...
  const currentLang = getCurrentLanguage();
  const currentLangBase = (currentLang.split('-')[0] || currentLang).toLowerCase();
  ...
  const filteredFeeds = feeds.filter(feed => {
    if (!feed.lang) return true;
    const feedLang = feed.lang.toLowerCase();
    return feedLang === currentLang.toLowerCase() || feedLang === currentLangBase;
  });

  const candidateFeeds = filteredFeeds.length > 0 ? filteredFeeds : feeds;
  const batches = chunkArray(candidateFeeds, batchSize);
