...
export type RuntimeSecretKey =
  ...
  | 'OLLAMA_API_URL'
  | 'OLLAMA_MODEL'
  /** Generic OpenAI-compatible key; OpenRouter falls back to this when OPENROUTER_API_KEY is unset. */
  | 'LLM_API_KEY'
  | 'LLM_API_URL'
...
export const RUNTIME_FEATURES: RuntimeFeatureDefinition[] = [
  ...
  },
  {
    ...
    fallback:
      'Also accepts LLM_API_URL pointing at openrouter.ai + LLM_API_KEY, or LLM_API_KEY alone if it is an OpenRouter sk-or-v1 key.',
  },
  {
    id: 'aiLlmGeneric',
...
export function isFeatureAvailable(featureId: RuntimeFeatureId): boolean {
  ...
  if (featureId === 'aiOpenRouter') {
    if (getSecretState('OPENROUTER_API_KEY').valid) return true;
    const llmKey = getSecretState('LLM_API_KEY');
    if (!llmKey.present || !llmKey.valid) return false;
    const raw = runtimeConfig.secrets.LLM_API_KEY?.value.trim() ?? '';
    const llmUrl = getSecretState('LLM_API_URL');
    if (llmUrl.present && llmUrl.valid && runtimeConfig.secrets.LLM_API_URL?.value) {
      try {
        if (new URL(runtimeConfig.secrets.LLM_API_URL.value.trim()).hostname.includes('openrouter.ai')) {
          return true;
        }
      } catch {
        /* ignore */
